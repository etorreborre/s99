...
class BinaryTreesSpec extends BinaryTreesProblems { def is =
 """
   ...
   *Construct completely balanced binary trees*
   ...
   number of nodes and a single value to put in all of them"""                                                          ! P55^
  """
   *Symmetric binary trees*
   ...
   We are only interested in the structure, not in the contents of the nodes"""                                         ! P56^
  """
   *Binary search trees (dictionaries)*
   ...
   Finally, use that function to test your solution to P56"""                                                           ! P57^
  """
   *Generate-and-test paradigm*

   Apply the generate-and-test paradigm to construct all symmetric, completely balanced
   binary trees with a given number of nodes"""                                                                         ! P58^
  """
   *Construct height-balanced binary trees*
   ...
   given height with a supplied value for the nodes. The function should generate all solutions"""                      ! P59^
  """
   *Construct height-balanced binary trees with a given number of nodes*

   Consider a height-balanced binary tree of
   height H. What is the maximum number of nodes it can contain? Clearly, MaxN = 2H - 1. However, what is the minimum
   number MinN? This question is more difficult. Try to find a recursive statement and turn it into a function
   `minHbalNodes` that takes a height and returns MinN.

  On the other hand, we might ask: what is the maximum height H a height-balanced binary tree with N nodes can have?
    Write a maxHbalHeight function.
  Now, we can attack the main problem: construct all the height-balanced binary trees with a given nuber of nodes.
  Find out how many height-balanced trees exist for N = 15"""                                                           ! P60^
  """
   *Count the leaves of a binary tree*

   A leaf is a node with no successors. Write a method leafCount to count them."""                                      ! P61^
  """
   *Collect the leaves of a binary tree in a list*

   A leaf is a node with no successors. Write a method leafList to
   collect them in a list"""                                                                                            ! P61A^
  """
   *Collect the internal nodes of a binary tree in a list*

   An internal node of a binary tree has either one or two
   non-empty successors. Write a method internalList to collect them in a list"""                                       ! P62^
  """
   *Collect the nodes at a given level in a list*\

   A node of a binary tree is at level N if the path from the root to
   the node has length N-1. The root node is at level 1. Write a method atLevel to collect all nodes at a given level
   in a list.
   Using `atLevel` it is easy to construct a method levelOrder which creates the level-order sequence of the nodes.
   However, there are more efficient ways to do that"""                                                                 ! P62B^
  """
   *Construct a complete binary tree*

   A complete binary tree with height H is defined as follows: The levels
   1,2,3,...,H-1 contain the maximum number of nodes (i.e 2(i-1) at the level i, note that we start counting the levels
   from 1 at the root). In level H, which may contain less than the maximum possible number of nodes, all the nodes are
   "left-adjusted". This means that in a levelorder tree traversal all internal nodes come first, the leaves come
   second, and empty successors (the Ends which are not really nodes!) come last.
  ...
  `completeBinaryTree` that takes as parameters the number of nodes and the value to put in each node"""                ! P63^
  """
   *Layout a binary tree (1)*

   As a preparation for drawing a tree, a layout algorithm is required to determine the
   position of each node in a rectangular grid. Several layout methods are conceivable, one of them is shown in the
   illustration on the right. In this layout strategy, the position of a node v is obtained by the following two rules:
   ...
   Use it to check your code"""                                                                                         ! P64^
  """
   *Layout a binary tree (2)*

   An alternative layout method is depicted in the illustration opposite. Find out the rules
   and write the corresponding method. Hint: On a given level, the horizontal distance between neighboring nodes is
   constant. Use the same conventions as in problem P64

   The tree at right may be constructed with `Tree.fromList(List('n','k','m','c','a','e','d','g','u','p','q'))`.
   Use it to check your code"""                                                                                         ! P65^
  """
   *Layout a binary tree (3)*
   ...
   Which layout do you like most?"""                                                                                    ! P66^
  """
   *A string representation of binary trees*
   ...
   For simplicity, suppose the information in the nodes is a single letter and there are no spaces in the string"""     ! P67^
 """
  *Preorder and inorder sequences of binary trees*
  ...
  `Tree.preInTree(List('a', 'b', 'a'), List('b', 'a', 'a'))`"""                                                         ! P68^
  """
   *Dotstring representation of binary trees*
   ...
   methods, `toDotstring` and `fromDotstring`, which do the conversion in both directions"""                            ! P69^
                                                                                                                        end
